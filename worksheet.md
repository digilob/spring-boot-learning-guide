...
* Notice the following dependency in the pom.xml:
  ...
  >      </dependency>    
* The "health" endpoint is automatically enabled, but is currently disabled in the applicaiton.yml file. Modify the health endpoint configuration in the application.yml and enable the health endpoint. Your management sections should now look like this:
  >     management:
  >       endpoint:
  >         health:
  >           enabled: true
...
* Modify the application.yml file again and enable the db and diskspace health details. You'll also need to add the "show-details" options set to "always". Your management section should now look like this:
  >     management:
  >       health:
  >         db:
  >           enabled: true
...
* Run _HealthTest_ again - All of the tests should be passing.
...
* We will come back to adding in your own HealthIndicator in a later step.
